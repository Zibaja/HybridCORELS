...
sensitive_hispanic_name = 'Race=Caucasian'
...
n = X_train.shape[0]
b=716 #bootstrap seed
np.random.seed(b)  # different seed each time
# sample indices with replacement
indices = np.random.choice(n, size=n, replace=True)
X_boot = {"train": X_train[indices], "test": X_test} 
y_boot = {"train": y_train[indices], "test": y_test}
#sensitive_train = (eval.cond_indices).astype(np.uint8)
...
bbox = RandomForestClassifier(random_state=716, min_samples_split=10, max_depth=10)
...
hyb_model.fit(
    X_boot['train'],
    y_boot['train'],
    sensitive_train=sensitive_boot_train,
    features=features,
...
preds_train, pred_types_train = hyb_model.predict_with_type(X_boot['train'])
cover_rate_train = float(np.mean(pred_types_train == 1))
train_group_cov, train_disparity = compute_coverage_stats(
    pred_types_train,
    sensitive_boot_train,
    ["African-American", "Hispanic", "Other"],
)
print("=> Training accuracy =", float(np.mean(preds_train == y_boot['train'])))
...
# preds_test, pred_types_test = hyb_model.predict_with_type(X['test'])
...
#     ["African-American", "Hispanic", "Other"],
# )
# print("=> Test accuracy =", float(np.mean(preds_test == y['test'])))
